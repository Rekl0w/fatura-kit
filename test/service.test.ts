...
import type {
  TransportRequest,
  TransportResponse,
} from "../src/client/portal-http-client";
...
function createMockTransport() {
  const documents = structuredClone(documentsFixture) as Array<
  ...
  return async <T>(
    ...
    if (request.url.endsWith("/dispatch")) {
      ...
      const payload = JSON.parse(String(request.form?.jp ?? "{}")) as Record<
        string,
        unknown
      >;
      ...
      switch (cmd) {
        ...
        case "EARSIV_PORTAL_TELEFONNO_SORGULA":
          return {
            status: 200,
            data: { data: { telefon: "5551234567" } } as T,
          };
        ...
        case "EARSIV_PORTAL_GELEN_IPTAL_ITIRAZ_TALEPLERINI_GETIR":
          return {
            status: 200,
            data: { data: [{ ettn: "request-1", durum: "BEKLIYOR" }] } as T,
          };
        case "EARSIV_PORTAL_IPTAL_TALEBI_OLUSTUR":
          return {
            status: 200,
            data: { data: "İptal talebi oluşturuldu" } as T,
          };
        case "EARSIV_PORTAL_ITIRAZ_TALEBI_OLUSTUR":
          return {
            status: 200,
            data: { data: "İtiraz talebi oluşturuldu" } as T,
          };
        case "EARSIV_PORTAL_FATURA_OLUSTUR": {
          ...
          return {
            status: 200,
            data: { data: "Belge başarıyla oluşturuldu" } as T,
          };
        }
        ...
        case "EARSIV_PORTAL_FATURA_GETIR": {
          ...
          return {
            status: 200,
            data: { data: storedInvoices.get(uuid) ?? {} } as T,
          };
        }
        case "EARSIV_PORTAL_MUSTAHSIL_GETIR": {
          ...
          return {
            status: 200,
            data: { data: storedProducerReceipts.get(uuid) ?? {} } as T,
          };
        }
        case "EARSIV_PORTAL_SERBEST_MESLEK_GETIR": {
          ...
          return {
            status: 200,
            data: { data: storedSelfEmployedReceipts.get(uuid) ?? {} } as T,
          };
        }
...
describe("service parity", () => {
  ...
  it("lists documents with upstream summary keys and selectColumn support", async () => {
    ...
    const selected = (await service
      .selectColumn("ettn")
      .getAll("01/09/2022", "15/09/2022")) as string[];
  ...
  it("handles html, download url, saving, phone and sms flows", async () => {
    ...
    expect(
      await service.completeSmsVerification("123456", "oid-123", [
        crypto.randomUUID(),
      ]),
    ).toBe(true);
    ...
    const savedPath = await service.saveToDisk(
      crypto.randomUUID(),
      tempDir,
      "document",
    );
  ...
  it("handles requests, cancellation and objection flows", async () => {
    ...
    expect(
      await service.cancellationRequest(crypto.randomUUID(), "Hatalı belge"),
    ).toContain("İptal");
    expect(
  ...
  it("creates, updates, and deletes an invoice draft", async () => {
    ...
    expect(createdInvoice.malhizmetToplamTutari).toBe(
      invoiceData.malhizmetToplamTutari,
    );
    expect(createdInvoice.matrah).toBe(invoiceData.matrah);
    expect(createdInvoice.toplamIskonto).toBe(invoiceData.toplamIskonto);
    expect(createdInvoice.vergilerDahilToplamTutar).toBe(
      invoiceData.vergilerDahilToplamTutar,
    );
  ...
  it("creates and retrieves producer receipt drafts", async () => {
    ...
    const service = new GibClient({
      transport: createMockTransport(),
      documentType: DocumentType.ProducerReceipt,
    });
    await service.setTestCredentials("33333310", "1");
    await service.login();
  ...
  it("creates and retrieves self-employed receipt drafts", async () => {
    ...
    const service = new GibClient({
      transport: createMockTransport(),
      documentType: DocumentType.SelfEmployedReceipt,
    });
    await service.setTestCredentials("33333310", "1");
    await service.login();
